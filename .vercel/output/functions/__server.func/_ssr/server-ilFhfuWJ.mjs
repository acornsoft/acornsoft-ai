...
import { n as ensureDbReady, r as getPglite } from "./db-cbgaf9gY.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/server-ilFhfuWJ.js
...
var vercelPublicUrl = env$1("VERCEL_URL") ? `https://${env$1("VERCEL_URL").replace(/^https?:\/\//, "")}` : void 0;
var explicitBaseURL = env$1("BETTER_AUTH_URL") ?? vercelPublicUrl;
...
*/
function mapBrokerProfile(providerId, profile) {
	const str = (k) => {
		const v = profile[k];
	...
	if (providerId === "grok-x") {
		const handle = str("preferred_username") ?? str("username") ?? str("screen_name") ?? str("nickname");
		return {
			name: handle ? handle.replace(/^@+/, "") : str("name") ?? str("display_name"),
			email,
			image
		};
	}
...
var grokOAuthPlugin = authConfigured ? genericOAuth({ config: GROK_PROVIDERS.map(({ providerId, idp }) => ({
	...
	mapProfileToUser: (profile) => mapBrokerProfile(providerId, profile)
})) }) : null;
var auth = betterAuth({
	baseURL,
