 ...
 * Baking it here lets the live preview do REAL sign-in — no demo users — with
 * no platform injection. When deployed the deployer injects a per-app
 * `GROK_AUTH_*` that overrides these (see `server.ts`).
 *
 * These MUST equal the broker's `GROK_PREVIEW_CLIENT_ID` /
 * `GROK_PREVIEW_CLIENT_SECRET`. Dedicated, low-privilege, preview-only.
 */
